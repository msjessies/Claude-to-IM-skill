...
import type { BridgeStatus, InboundMessage, OutboundMessage, StreamingPreviewState } from 'claude-to-im/src/lib/bridge/types.js';
...
/** Bridge-managed slash commands that should be intercepted locally. */
...
function isBridgeCommand(rawText: string): boolean {
...
import type { ChannelAddress, SendResult } from 'claude-to-im/src/lib/bridge/types.js';
...
async function handleMessage(
  ...
  // Check for bridge-managed IM commands (before sanitization — commands are validated individually)
  if (isBridgeCommand(rawText)) {
...
/** @internal */
export const _testOnly = { handleMessage, isBridgeCommand, isNumericPermissionShortcut };
